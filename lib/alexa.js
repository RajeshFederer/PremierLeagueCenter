...
skills.schedule = (req, res) =>{
    ...
    .then((finalSlotData)=>{
        ...
        if(finalSlotData.matchDay){
            console.log('DUDE', finalSlotData.matchDay.value, moment().isBefore(finalSlotData.matchDay.value), moment().isBefore(moment(finalSlotData.matchDay.value)));
            if(moment().isBefore(finalSlotData.matchDay.value)){
                console.log('INN ');
                return res.json({"response": {
                    "outputSpeech": {
                        "type": "PlainText",
