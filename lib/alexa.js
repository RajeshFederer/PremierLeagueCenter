...
skills.schedule = (req, res) =>{
    ...
    .then((finalSlotData)=>{
        ...
        if(finalSlotData.matchDay){
            console.log('DUDE', finalSlotData.matchDay.value, moment().startOf('day').isAfter(finalSlotData.matchDay.value), moment().startOf('day').isAfter(moment(finalSlotData.matchDay.value)));
            if(moment().startOf('day').isAfter(finalSlotData.matchDay.value)){
                console.log('INN ');
                return res.json({"response": {
                    "outputSpeech": {
                        "type": "PlainText",
...
function formScheduleResponse(schedules, finalSlotData){
    ...
    if(schedules && schedules.length){
        ...
        _.each(schedules, (sched) =>{
            sched.formatted_date = sched.formatted_date.replace(/\./g,"-");
            console.log(sched);
        });
