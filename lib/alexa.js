    ...
    return new Promise((resolve, reject) =>{
        if(finalSlotData.team){
            ...
            if(finalSlotData.tableSide){
                let position = finalSlotData.tableSide.toLowerCase() == "bottom" ? 20 :1;
                if(finalSlotData.positions){
                    position = finalSlotData.tableSide.toLowerCase() == "bottom" ? 20 -finalSlotData.positions : finalSlotData.tableSide;
                }
...
function filterTable(standings,key, value){
    ...
    let data = standings.find(element =>{
        console.log('AlLU',element[key], value.toString());
