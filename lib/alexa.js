...
function getSlotValue(slotValues, reqSlots){
    return new Promise((resolve, reject) =>{
        ...
        async.forEachSeries(slotValues, function(soltObj, cback){
            let slot = soltObj;
            let reqSlotValue = reqSlots[slot.name] && slot.name ? reqSlots[slot.name].value: null;
            if(reqSlotValue){
