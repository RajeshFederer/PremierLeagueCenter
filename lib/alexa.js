...
function getSlotValue(slotValues, reqSlots){
    return new Promise((resolve, reject) =>{
        let finalSlotData = {};
        slotValues.forEach(element, inde => {
            if(!element.name){
                console.log(element, inde);
            }
            console.log('PAL ',element.name);
            let reqSlotValue = reqSlots[element.name].value;
            if(reqSlotValue){
                let matchingSlot = element.values.filter((slotData) =>{
                    return slotData.name.synonyms.indexOf(reqSlotValue) !== -1;
                });
                console.log(matchingSlot, reqSlotValue);
                finalSlotData[element.name] = matchingSlot[0].name.value;
            }
        });
